...
mkdir -p "preliminary/orthogroupsdisco_nu"

# Iterate over each line in the input file
while IFS= read -r line; do
    name=$((name+1))
    output_file="preliminary/orthogroupsdisco_nu/${name}.fa"
    ...
    echo "$line" | grep -o "[A-Za-z]\{3\}_[A-Z]\{3\}_[0-9]\{5\}-R[A-Z]" | while IFS= read -r full_id; do
        ...
        fasta_file="preliminary/AGAT/nucleotides/renamed_one/nu_sequences_${prefix}.fasta"
